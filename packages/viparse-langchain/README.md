...
**LangChain integration for [viparse](https://github.com/TrizenX/viparse)** — read Vietnamese
...
```python
from viparse_langchain import VietnameseDocumentLoader
...
with `pip install "viparse[langchain]"`; this distribution exists so that a LangChain user
can **find** it, because LangChain no longer accepts third-party integrations into its own
repositories.

## Why not `langchain-viparse`

That prefix belongs to LangChain's own packages. Using it would imply an official
